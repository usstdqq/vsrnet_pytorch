...
train_set = DatasetFromH5_SFSR('/home/dqq/SR_ZOO/Data/CDVL/uf_4/train/Data_CDVL_LR_Bic_MC_uf_4_ps_72_fn_5_tpn_225000.h5',
...
val_set = DatasetFromH5_SFSR('/home/dqq/SR_ZOO/Data/CDVL/uf_4/val/Data_CDVL_LR_Bic_MC_uf_4_ps_72_fn_5_tpn_45000.h5',
...
model = Net_SRCNN(upscale_factor=opt.upscale_factor)
...
configure("tensorBoardRuns/SRCNN-relu-9-5-5-batch-256-CDVL-225000x5x72x72-wd")
...
def train(epoch):
    ...
    # The network has 3 layers, and the learning rate of the last layer is 10 times smaller than the other two layers
...
def val(epoch):
    ...
    print("===> Epoch {} Validation ImageNet: Avg. Loss: {:.4f}, Avg.PSNR:  {:.4f} dB, Time: {:.4f}".format(epoch, avg_mse / frame_count, avg_psnr / frame_count, (end-start)))
...
def checkpoint(epoch):
    if epoch%10 == 0:
        if not os.path.exists("epochs_SRCNN"):
            os.makedirs("epochs_SRCNN")
        model_out_path = "epochs_SRCNN/" + "model_epoch_{}.pth".format(epoch)
